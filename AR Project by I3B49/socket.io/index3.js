...
io.on('connect', function(socket){
  ...
  status.push ({id: nID, turn: false, number: 0.01});
  console.log (status);
  ...
  socket.on('toggle', function(myID) {
	...
	socket.on('toggle_speed', function(myID, speed) {
  	console.log (myID + ': toggle speed');
  	// find myID in status
  	let i;
  	for (i = 0; i < status.length; i++) {
  		if (status[i].id === myID) break;
  	}
		status[i].number = speed;
	 	console.log (status);
  	io.emit ('update_status_speed', status);
  });
